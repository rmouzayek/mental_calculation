...
class Model:
    ...
    def __init__(self, limit_score, born_sup, nbr_dec, operations_allowed):
        self.limit_score = int(limit_score); self.born_sup = float(born_sup); self.nbr_dec = int(nbr_dec)
        self.operations_allowed = operations_allowed
        self.score = 0; self.nbr_rounds_played = 0 
        self.success = False; self.still_playing = True # initialize states
        
    def generate_challenge(self):
        ''' Updates the current calculation, and returns it for the Controller. '''
        ope = generate_operation(self.operations_allowed)
        nbr1 = generate_number(self.born_sup, self.nbr_dec); nbr2 = generate_number(self.born_sup, self.nbr_dec)
    ...
    def assess_answer(self, user_answer):
        ...
        user_answer (string) value entered by the user and collected by the Controller.'''
